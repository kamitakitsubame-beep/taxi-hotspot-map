...
# さいたまスーパーアリーナ（大型公演・スポーツ）の予定表
ARENA_URL = "https://www.saitama-arena.co.jp/schedule/"
ARENA_LATLNG = (35.8951, 139.6306)   # 本体アリーナ
KEYAKI_LATLNG = (35.8966, 139.6310)  # けやきひろば
# 手動で追記する高需要イベント（レッズ/大宮の試合など。自動取得できない分の補完）
...
# アリーナ：物販・展示・オフィス等は需要が低いため除外
ARENA_EXCLUDE = ["フリーマーケット", "フリマ", "展示会", "バーゲン", "販売会",
                 "即売会", "オフィス", "コワーキング", "セミナー", "クーポン",
                 "物販", "商談", "見本市", "受付", "献血", "予約制"]
# アリーナ：コンサート・スポーツのキーワード（これらのみ採用）
ARENA_CONCERT = ["ライブ", "LIVE", "TOUR", "ツアー", "コンサート", "公演",
                 "フェス", "FES", "Bリーグ", "バスケ", "試合", "格闘技",
                 "ボクシング", "プロレス", "ワンマン", "音楽", "ホール公演"]
ARENA_BIG_HALL = ["メインアリーナ", "スタジアム", "ホール"]  # 本体＝大需要
...
# --------------------------------------------------------------------------- #
# Walkerplus 詳細ページから開催時刻を取得
# --------------------------------------------------------------------------- #
TIME_RE = re.compile(r"(\d{1,2}):(\d{2})")


def fetch_times(session: requests.Session, href: str) -> tuple[str | None, str | None]:
    """詳細ページの「開催時間 14:00〜15:00」「開場13:30」から開始・終了時刻を返す。"""
    try:
        r = session.get(SITE + href, timeout=REQUEST_TIMEOUT)
        r.encoding = "utf-8"
        s = BeautifulSoup(r.text, "html.parser")
        start = end = None
        for el in s.select(".m-detailheader__period"):
            t = el.get_text(" ", strip=True)
            if "開催時間" in t:
                ts = TIME_RE.findall(t)
                if ts:
                    start = f"{int(ts[0][0]):02d}:{ts[0][1]}"
                if len(ts) > 1:
                    end = f"{int(ts[1][0]):02d}:{ts[1][1]}"
        if not start:  # 開催時間が無ければ開場時刻で代用
            for el in s.select(".m-detailheader__period, .m-detailheader__text"):
                t = el.get_text(" ", strip=True)
                if "開場" in t:
                    m = TIME_RE.search(t)
                    if m:
...
# --------------------------------------------------------------------------- #
# さいたまスーパーアリーナ（大型公演・スポーツ）
# --------------------------------------------------------------------------- #
ARENA_DATE_RE = re.compile(
    r"開催日：\s*(\d{4}/\d{1,2}/\d{1,2})\([^)]*\)"
    r"\s*(?:～\s*(\d{4}/\d{1,2}/\d{1,2})\([^)]*\))?\s*(.+?)"
    r"\s*(?:入場料|お問合せ|会場タイプ|$)"
)
ARENA_HALL_RE = re.compile(r"会場タイプ\s*([^\s　]+)")


def _arena_level(title: str, hall: str) -> str | None:
    if any(k in title for k in ARENA_EXCLUDE):
        return None
    big = any(k in hall for k in ARENA_BIG_HALL)
    concert = any(k in title for k in ARENA_CONCERT)
    if big and (concert or "アリーナ" in hall or "スタジアム" in hall):
        return "high"
    if concert:
        return "medium"
    return None
...
def scrape_arena(session: requests.Session, today: dt.date,
                 window_end: dt.date) -> list[dict]:
    try:
        r = session.get(ARENA_URL, timeout=REQUEST_TIMEOUT)
    ...
    for li in soup.select("li"):
        ...
        m = ARENA_DATE_RE.search(txt)
        if not m:
            continue
        try:
            start = dt.datetime.strptime(m.group(1), "%Y/%m/%d").date()
            end = (dt.datetime.strptime(m.group(2), "%Y/%m/%d").date()
                   if m.group(2) else start)
        except ValueError:
            continue
        title = m.group(3).strip()
        hall_m = ARENA_HALL_RE.search(txt)
        hall = hall_m.group(1) if hall_m else ""
        level = _arena_level(title, hall)
        if level is None:
            continue
        if not (start <= window_end and end >= today):
            continue
        if (end - start).days > 21:  # 長期は除外
            continue
        key = (title, start)
        if key in seen:
            continue
        ...
        keyaki = "けやき" in hall
        lat, lng = KEYAKI_LATLNG if keyaki else ARENA_LATLNG
        place = "けやきひろば" if keyaki else f"さいたまスーパーアリーナ {hall}".strip()
        comment = (
            "さいたまスーパーアリーナの大型公演。終演後にさいたま新都心駅周辺で大量需要。早めの待機を強く推奨。"
            if level == "high" else
            "けやきひろば等での催し。来場者の行き帰りで新都心駅周辺に需要が見込まれます。"
        )
        disp = start if start >= today else today
        rows.append({
            "id": f"arena-{abs(hash((title, start.isoformat()))) % 10**8}",
            "title": title[:80],
            "date": disp.isoformat(),
            "date_end": end.isoformat(),
            "venue": f"{place}（さいたま市中央区）",
            "category": "コンサート・スポーツ",
            "demand_level": level,
            "demand_comment": comment,
...
# --------------------------------------------------------------------------- #
# 手動の高需要イベント（data/fixtures.json）
# --------------------------------------------------------------------------- #
def load_fixtures(today: dt.date, window_end: dt.date) -> list[dict]:
    if not FIXTURES_FILE.exists():
        return []
    try:
        data = json.loads(FIXTURES_FILE.read_text(encoding="utf-8"))
    except (json.JSONDecodeError, OSError) as exc:
        log(f"fixtures読込失敗（無視）: {exc}")
        return []
    out = []
    for e in data.get("events", []):
        try:
            ...
            continue
        if today <= d <= window_end and isinstance(e.get("lat"), (int, float)):
            out.append(e)
    return out
...
def main() -> int:
    ...
    prev_auto = [e for e in ex_events
                 if str(e.get("id", "")).startswith(("wp-", "arena-"))]
    ...
    # 既存の時刻をキャッシュ（詳細ページの再取得を避ける）
    time_cache = {
        e["id"]: (e.get("time_start"), e.get("time_end"))
        for e in ex_events if str(e.get("id", "")).startswith("wp-")
    }

    # --- Walkerplus（エリア内イベント＋時刻） ---
    try:
        scraped = scrape(session)
        log(f"Walkerplus 取得: 対象エリア {len(scraped)} 件")
    except Exception as exc:  # noqa: BLE001 — 全体は落とさない
        log(f"Walkerplusスクレイプ失敗: {exc}")
        scraped = []

    if not scraped:
        wp_resolved = [e for e in prev_auto if str(e.get("id", "")).startswith("wp-")]
        log("Walkerplus 0件のため前回データを維持します。")
    else:
        ...
        for ev in scraped:
            ...
            # 開催時刻：キャッシュ優先、無ければ詳細ページから取得
            cached = time_cache.get(ev["id"])
            if cached and cached[0]:
                ev["time_start"] = cached[0]
                if cached[1]:
                    ev["time_end"] = cached[1]
            else:
                ts, te = fetch_times(session, ev["source_url"].replace(SITE, ""))
                if ts:
                    ev["time_start"] = ts
                if te:
                    ev["time_end"] = te
                time.sleep(0.4)
            wp_resolved.append(ev)

    # --- さいたまスーパーアリーナ（大型公演・スポーツ） ---
    try:
        arena = scrape_arena(session, today, window_end)
        log(f"アリーナ 取得: {len(arena)} 件")
    except Exception as exc:  # noqa: BLE001
        log(f"アリーナ取得失敗: {exc}")
        arena = [e for e in prev_auto if str(e.get("id", "")).startswith("arena-")]

    # --- 手動の高需要イベント（試合など） ---
    fixtures = load_fixtures(today, window_end)
    if fixtures:
        log(f"手動(fixtures) 取得: {len(fixtures)} 件")

    events_out = kept_manual + fixtures + wp_resolved + arena
    events_out.sort(key=lambda e: (e.get("date", ""),
                                   e.get("time_start", "99:99"),
                                   e.get("title", "")))
    ...
    log(f"書き出し完了: {len(events_out)} 件 "
        f"(WP{len(wp_resolved)} / アリーナ{len(arena)} / 手動{len(kept_manual) + len(fixtures)})")
    return 0
